...
vector<string> col_confuse(vector<string> &s) {
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        // 先转成数字
        int s0 = str_long(temp[0]), s1 = str_long(temp[1]),
            s2 = str_long(temp[2]), s3 = str_long(temp[3]);
        // 计算混淆后的值
...
// 先定义一个 aes 加密函数
string aes(string A, string B) {
    string plain_text = A, key = B;
    // 先拓展密钥
...
vector<string> in_col_confuse(vector<string> &s) {
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        // 将数字转换成字符串存储
        ans[i] =
            int_ch2(t4[0]) + int_ch2(t4[1]) + int_ch2(t4[2]) + int_ch2(t4[3]);
    }
